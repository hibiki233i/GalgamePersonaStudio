...
基于游戏脚本的 Galgame 角色人格提取。可自动从脚本 JSON 中提取高频角色、分析台词特征并生成角色卡，解决直接询问 LLM 角色人格回答过于单薄 / 冷门角色没有数据。
...
- **选择肢检测**：连续多次无新文本后触发，通过宽区域 OCR + 正则识别选项（可配框选区域）
- **弹窗手动选择 / 自动选择**（第一条/最后一条）
- **打字机文字容错**：同角色渐显文字自动合并，不会误翻页
- **黑屏/过场跳过**：连续无文字时自动点击推进
- **可自定义快捷键**：开始/停止记录，全局生效（程序最小化亦可触发）
...
程序启动时自动检查 GitHub Releases 是否有新版本，提示用户前往下载页。
...
- [x] 选择肢识别与处理
- [ ] 更多游戏引擎兼容（Ren'Py / TyranoScript 等）
- [ ] 批量角色人格生成
